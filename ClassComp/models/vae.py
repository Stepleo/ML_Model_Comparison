...
class VAE_conv(UNet):
    ...
    """
    def __init__(self, input_img_size, resnet = None, beta=0.005, classification_mode=False):
        super(VAE_conv, self).__init__(input_img_size, resnet)
        # Setup for the reparametrization trick
        final_dim = input_img_size // 32
        self.fc_mu = nn.Linear(final_dim * final_dim, 8)
        self.fc_var = nn.Linear(final_dim * final_dim, 8)
        ...
        self.decoder_input = nn.ConvTranspose2d(in_channels=8, out_channels=512, kernel_size=final_dim)   
